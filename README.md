...
本作はC++を使わず、Blueprintのみで制作しました。理由は主に3つあります。

1つ目は、本作が自分にとって初めての本格的なUE5チーム制作だったことです。まずBlueprintでUE5のAI機能(Behavior Tree、Blackboard、AI Perception、EQS)の使い方を一通り学び、手を動かしながら仕組みを理解することを優先しました。

2つ目は、チームメンバーがBlueprintの方が触りやすかったことです。本作は4人チームで、メンバーにはエフェクトやUI、3Dモデルの担当がおり、C++を読み書きするメンバーは自分だけでした。BPならエディタ上でノードの流れが見えるので、他メンバーが敵の挙動を確認したり、パラメータを調整するときのハードルが低くなります。

3つ目は、本作の規模ではBlueprintでも十分に動作したことです。敵タイプは4種(近接・遠距離・魔法・ボス)で、処理負荷的にC++でなければ困る場面は出ませんでした。

一方で、TaskやDecoratorが増えるにつれてBPグラフの全体像が追いにくくなる問題も実感しました。この経験が、次の制作(Wipe&Snap)でLeap Motion入力のロジック中核をC++に寄せる判断につながっています。
...
| コード生成・調査支援 | Claude Code (Anthropic) | パッケージ化エラーの原因調査、設定ファイル修正案の確認 |

最終的な仕様判断、Blueprint の設計・実装、Behavior Tree 構造の設計、EQS 設定、敵AIの仕様判断、マップ配置、ゲームプレイの仕様決定は自分(およびチームメンバー)で行っています。生成素材については、提出物で使用可能な範囲か確認したうえで利用しています。
