...
敵AIでは、共通AI Controllerがプレイヤーを視覚・聴覚・被ダメージで検知し、Blackboardへターゲットや状態を保存します。Behavior Treeはその値を見て、巡回、探索、追跡、攻撃、回復、テレポートなどを切り替えます。近接・遠距離・魔法・ボスでBTを分けたことで、共通の状態管理を使いながら敵ごとの個性を出せる構成にしました。実装時はUE5のAI機能(Behavior Tree、Blackboard、AI Perception、EQS)の使い方を解説記事や動画で学びながら、参考にした処理をそのまま置くだけでなく、本作の敵挙動に合わせてBlackboard key、BT分岐、EQS用途、攻撃Taskを調整しました。
